...
const schema = `
  ...
  CREATE INDEX IF NOT EXISTS videos_keywords_idx ON videos(keywords);
...
export class CatalogDatabase {
  ...
  constructor(file) {
    ...
    if (!columns.some((column) => column.name === 'keywords'))
      this.#db.exec("ALTER TABLE videos ADD COLUMN keywords TEXT NOT NULL DEFAULT ''");

    const analysisColumns = this.#db.prepare('PRAGMA table_info(video_analysis)').all();
    if (!analysisColumns.some((column) => column.name === 'retry_count'))
      this.#db.exec('ALTER TABLE video_analysis ADD COLUMN retry_count INTEGER NOT NULL DEFAULT 0');

    const searchColumns = this.#db.prepare('PRAGMA table_info(video_search)').all();
    if (!searchColumns.some((column) => column.name === 'keywords')) {
    ...
    this.#upsertVideo = this.#db.prepare(`
      ...
      ON CONFLICT(file_path) DO UPDATE SET
        ...
        keywords = CASE WHEN excluded.keywords != '' THEN excluded.keywords ELSE videos.keywords END
    ...
    this.#insertSearch = this.#db.prepare(
      'INSERT INTO video_search (video_id, title, description, summary, tags, people, location, keywords) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
    );
  ...
  updateVideoKeywords(videoId, keywords) {
    const timestamp = now();
    this.#db.prepare('UPDATE videos SET keywords = ? WHERE id = ?').run(keywords, videoId);
    ...
    this.#db
      .prepare(
        `INSERT INTO video_analysis (video_id, status, summary, provider, model, analyzed_at, error, retry_count)
         VALUES (?, 'completed', ?, 'ollama', 'moondream', ?, NULL, 0)
      ...
      .run(videoId, keywords, timestamp);
  }

  recordAnalysisFailure(videoId, error) {
    const timestamp = now();
    this.#db
      .prepare(
        `INSERT INTO video_analysis (video_id, status, summary, provider, model, analyzed_at, error, retry_count)
         VALUES (?, 'failed', '', 'ollama', 'moondream', ?, ?, 1)
         ON CONFLICT(video_id) DO UPDATE SET
           status = 'failed',
           analyzed_at = excluded.analyzed_at,
      ...
      .run(videoId, timestamp, error);
  }

  resetAiAnalysis() {
    this.#db.exec(`
      BEGIN;
      UPDATE videos SET keywords = '';
      UPDATE video_analysis
      SET status = 'pending', summary = '', provider = NULL, model = NULL,
          analyzed_at = NULL, error = NULL, retry_count = 0;
      DELETE FROM video_search;
      INSERT INTO video_search (video_id, title, description, summary, tags, people, location, keywords)
      SELECT id, title || ' ' || filename, description, '', '', people, location, ''
      FROM videos;
      COMMIT;
    `);
  ...
  getAiStatus() {
    const totalRow = this.#db.prepare('SELECT COUNT(*) as count FROM videos').get();
    const processedRow = this.#db
      .prepare("SELECT COUNT(*) as count FROM videos WHERE keywords != ''")
      .get();
    const errorRow = this.#db
      .prepare(
        "SELECT COUNT(*) as count FROM video_analysis WHERE status = 'failed' AND retry_count >= 2",
      )
      .get();
    ...
    return {
      ...
      processed,
      errors,
      remaining: Math.max(0, total - processed - errors),
    };
  }
